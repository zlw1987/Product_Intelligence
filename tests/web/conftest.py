"""Dedicated DB isolation for web/ tests.
...
rely on their own teardown (Django TestCase transaction rollback, or
Django test client session isolation).

The fixture is narrowly scoped to the tables used by web/ tests only.
"""
...
def human_review_db_isolation(request) -> None:
    """Dedicated DB isolation for HUMAN-REVIEW web tests.
    ...
    Usage::
        ...
        def test_something(client, human_review_db_isolation) -> None:
            ...
    """
    yield  # Run the test
