...
class TestIntegrityErrorRecovery(TestCase):
    ...
    1. expected active-slot conflict -> create() is called and raises,
       recovery finds winning child
    2. unrelated IntegrityError -> exact IntegrityError propagates

    The tests must PROVE the collision-recovery branch executes. Pre-creating
    a winner before calling trigger_comparable_research() would cause the
    optimistic pre-check to return it without ever calling create(), so the
    collision branch would not be exercised. We patch filter() so the
    optimistic pre-check returns None, then patch create() to raise
    IntegrityError, while a pre-created winner exists for the recovery query.
    """

    def test_collision_recovery_branch_exercises_create_path(self) -> None:
        """When the UNIQUE constraint fires, the winning active child is
        returned. The create() path must be called and the recovery branch
        ...
        create_called = [False]
        orig_filter = ComparableResearchExecution.objects.filter
        call_n = [0]
        ...
        def mock_filter(*a, **kw):
            if 'state__in' in kw:
                call_n[0] += 1
                if call_n[0] == 1:
                    return ComparableResearchExecution.objects.none()
            return orig_filter(*a, **kw)

        with patch.object(ComparableResearchExecution.objects, 'create', mock_create):
            with patch.object(ComparableResearchExecution.objects, 'filter', mock_filter):
    ...
    def test_collision_recovery_returns_running_winner(self) -> None:
    ...
    def test_collision_recovery_returns_completed_winner(self) -> None:
        """Collision recovery returns a COMPLETED winner when the
        optimistic pre-check sees no active child."""
        from unittest.mock import patch
        ...
        claim_comparable_research(child.id)
        complete_comparable_research(child.id, 1, {"kind": "FULL"})
        winner = ComparableResearchExecution.objects.get(id=child.id)
        ...
        def mock_filter(*a, **kw):
            # Intercept both state__in and state=COMPLETED in optimistic checks
            if 'state__in' in kw:
                call_n[0] += 1
                if call_n[0] == 1:
                    return ComparableResearchExecution.objects.none()
            elif kw.get('state') == ComparableResearchState.COMPLETED:
                call_n[0] += 1
                if call_n[0] == 2:
                    # Second COMPLETED query (optimistic check): return empty
                    return ComparableResearchExecution.objects.none()
            return orig_filter(*a, **kw)
        ...
        assert create_called[0]
        assert result.id == winner.id
        assert result.state == ComparableResearchState.COMPLETED
